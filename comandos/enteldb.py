...
# =============== Comando principal ===============
async def enteldb_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de CELULAR después de /enteldb.", reply_to_message_id=update.message.message_id)
        return
    ...
    command_cfg = get_command_runtime_config("enteldb", 3)
    ...
    await create_request(update, context, "enteldb", cost=required_credits)