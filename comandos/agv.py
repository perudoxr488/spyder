...
async def agv_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de DNI después de /agv.", reply_to_message_id=update.message.message_id)
        return
    dni = context.args[0]
    ...
    command_cfg = get_command_runtime_config("agv", 20)
    required_credits = int(command_cfg.get("cost", 20))
    ...
    await create_request(update, context, "agv", cost=required_credits)