...
# =============== Comando principal ===============
async def tels_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de CELULAR después de /tels.", reply_to_message_id=update.message.message_id)
        return
    ...
    command_cfg = get_command_runtime_config("tels", 5)
    ...
    await create_request(update, context, "tels", cost=required_credits)