...
# =============== Comando principal ===============
async def sunat_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de RUC después de /sunat.", reply_to_message_id=update.message.message_id)
        return
    ruc = context.args[0]
    ...
    command_cfg = get_command_runtime_config("sunat", 8)
    ...
    await create_request(update, context, "sunat", cost=required_credits)