...
# =============== Comando principal ===============
async def trabajadores_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de RUC después de /trabajadores.", reply_to_message_id=update.message.message_id)
        return
    ruc = context.args[0]
    if not ruc.isdigit() or len(ruc) != 11:
        await update.message.reply_text("Por favor, introduce un número de RUC válido (11 dígitos).", reply_to_message_id=update.message.message_id)
        return
    ...
    command_cfg = get_command_runtime_config("trabajadores", 8)
    required_credits = int(command_cfg.get("cost", 8))
    ...
    loading_ft = (CMDS.get("FT_EXTRAS") or "").strip() or None
    loading_txt = (CMDS.get("TXT_EXTRAS") or "Consultando…").strip()
    try:
        if loading_ft:
    ...
    await create_request(update, context, "trabajadores", cost=required_credits)