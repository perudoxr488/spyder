...
# =============== Comando principal ===============
async def antpe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de DNI después de /antpe.", reply_to_message_id=update.message.message_id)
        return
    dni = context.args[0]
    ...
    command_cfg = get_command_runtime_config("antpe", 7)
    ...
    await create_request(update, context, "antpe", cost=required_credits)