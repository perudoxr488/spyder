...
# =============== Comando principal ===============
async def claro_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de CELULAR después de /claro.", reply_to_message_id=update.message.message_id)
        return
    ...
    command_cfg = get_command_runtime_config("claro", 7)
    ...
    await create_request(update, context, "claro", cost=required_credits)