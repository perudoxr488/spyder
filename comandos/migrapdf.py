...
# =============== Comando principal ===============
async def migrapdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de DNI después de /migrapdf.", reply_to_message_id=update.message.message_id)
        return
    dni = context.args[0]
    ...
    command_cfg = get_command_runtime_config("migrapdf", 6)
    required_credits = int(command_cfg.get("cost", 6))
    ...
    loading_ft = (CMDS.get("FT_MIGRACIONES") or "").strip() or None
    loading_txt = (CMDS.get("TXT_MIGRACIONES") or "Consultando…").strip()
    try:
        if loading_ft:
    ...
    await create_request(update, context, "migrapdf", cost=required_credits)