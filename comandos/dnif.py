...
# =============== Comando principal ===============
async def dnif_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de DNI después de /dnif.", reply_to_message_id=update.message.message_id)
        return
    dni = context.args[0]
    ...
    command_cfg = get_command_runtime_config("dnif", 3)
    ...
    loading_ft = (CMDS.get("FT_RENIEC") or "").strip() or None
    loading_txt = (CMDS.get("TXT_RENIEC") or "Consultando…").strip()
    try:
        if loading_ft:
    ...
    await create_request(update, context, "dnif", cost=required_credits)