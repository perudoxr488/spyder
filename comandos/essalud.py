...
# =============== Comando principal ===============
async def essalud_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de DNI después de /essalud.", reply_to_message_id=update.message.message_id)
        return
    dni = context.args[0]
    ...
    command_cfg = get_command_runtime_config("essalud", 3)
    required_credits = int(command_cfg.get("cost", 3))
    ...
    await create_request(update, context, "essalud", cost=required_credits)