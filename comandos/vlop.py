...
# =============== Comando principal ===============
async def vlop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Por favor, proporciona el número de CELULAR después de /vlop.", reply_to_message_id=update.message.message_id)
        return
    ...
    command_cfg = get_command_runtime_config("vlop", 1)
    ...
    await create_request(update, context, "vlop", cost=required_credits)